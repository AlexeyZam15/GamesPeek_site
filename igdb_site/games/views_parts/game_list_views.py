...
def get_similar_games_for_criteria(selected_criteria: Dict[str, List[int]], search_filters: Dict = None) -> Tuple[
    List, int]:
    """Get similar games for criteria with Redis caching."""
    ...
    similarity_engine = GameSimilarity(verbose=DEBUG_SIMILARITY)
    ...
    try:
        ...
        if cached_result_raw:
            ...
            _debug_print(f"Redis CACHE HIT for criteria - returning {cached_result['count']} games")
            _debug_print(f"Total time: {time.time() - start_total:.3f}s")
            _debug_print("=== get_similar_games_for_criteria END (Redis HIT) ===\n")
            return cached_result['games'], cached_result['count']
    except Exception as e:
        _debug_print(f"Redis cache read error: {e}")

    cached_data = cache.get(cache_key)
    _debug_print(f"Cache check: {time.time() - stage_start:.3f}s")

    if cached_data:
        logger.debug(f"Cache HIT for criteria search: {len(cached_data['games'])} games")
        _debug_print(f"Django Cache HIT - returning {len(cached_data['games'])} games")
        _debug_print(f"Total time: {time.time() - start_total:.3f}s")

        try:
            redis_client.setex(redis_cache_key, 300, pickle.dumps(cached_data))
            _debug_print(f"Saved to Redis cache for criteria")
        except Exception as e:
            _debug_print(f"Failed to save to Redis: {e}")

        _debug_print("=== get_similar_games_for_criteria END (Django HIT) ===\n")
        return cached_data['games'], cached_data['count']

    _debug_print(f"Cache MISS - calculating similarity...")
    ...
    _debug_print(f"VirtualGame created: {time.time() - stage_start:.3f}s")
    ...
    _debug_print(f"find_similar_games executed: {time.time() - stage_start:.3f}s, found {len(similar_games)} games")
    ...
    _debug_print(f"Django cache save: {time.time() - stage_start:.3f}s")
    ...
    _debug_print(f"TOTAL time for get_similar_games_for_criteria: {time.time() - start_total:.3f}s")
    _debug_print("=== get_similar_games_for_criteria END ===\n")
...
def get_similar_games_for_game(game_obj: Game, selected_platforms: List[int], search_filters: Dict = None) -> Tuple[
    List, int]:
    """Get similar games for a specific game with Redis caching."""
    from ..similarity import GameSimilarity
    import hashlib
    import json
    import pickle
    ...
    _debug_print(f"Search filters received: {search_filters}")
    ...
    try:
        ...
        if cached_result_raw:
            cached_result = pickle.loads(cached_result_raw)
            _debug_print(f"Redis CACHE HIT for game {game_obj.id} with filters {search_filters}")
            _debug_print(f"Total time: {time.time() - start_total:.3f}s")
            return cached_result[0], cached_result[1]
    except Exception as e:
        _debug_print(f"Redis cache read error: {e}")

    _debug_print(f"Cache MISS - calculating similarity with filters {search_filters}...")
    ...
    if search_filters:
        ...
        if year_start or year_end:
            _debug_print(f"VERIFYING date filter: year_start={year_start}, year_end={year_end}")
            for item in similar_games[:10]:
                ...
                if hasattr(game, 'first_release_date') and game.first_release_date:
                    ...
                    _debug_print(f"  Game: {game.name}, Year: {game_year}")

    total_count = len(similar_games)

    try:
        redis_client.setex(redis_cache_key, 300, pickle.dumps((similar_games, total_count)))
        _debug_print(f"Saved to Redis cache for game {game_obj.id}")
    except Exception as e:
        _debug_print(f"Failed to save to Redis: {e}")

    _debug_print(f"TOTAL time for get_similar_games_for_game: {time.time() - start_total:.3f}s")
    _debug_print("=== get_similar_games_for_game END ===\n")
