        ...
        try {
            ...
            if (applyButton) {
                ...
                newButton.addEventListener('click', (e) => {
                    ...
                    if (activeTab === 'search-filters-pane') {
                        // Search Filters - собираем значения из чекбоксов
                        const platformCheckboxes = document.querySelectorAll('.search-platform-checkbox:checked');
                        const platformIds = Array.from(platformCheckboxes).map(cb => cb.value);
                        ...
                        // ПРОСТО ДОБАВЛЯЕМ search параметры к существующему URL
                        if (platformIds.length > 0) {
                            currentUrl.searchParams.set('p', platformIds.join(','));
                        } else {
                            currentUrl.searchParams.delete('p');
                        }

                        if (gameTypeIds.length > 0) {
                            currentUrl.searchParams.set('gt', gameTypeIds.join(','));
                        } else {
                            currentUrl.searchParams.delete('gt');
                        }

                        if (yearStart && yearStart.trim() !== '') {
                            currentUrl.searchParams.set('ys', yearStart);
                        } else {
                            currentUrl.searchParams.delete('ys');
                        }

                        if (yearEnd && yearEnd.trim() !== '') {
                            currentUrl.searchParams.set('ye', yearEnd);
                        } else {
                            currentUrl.searchParams.delete('ye');
                        }

                        // ВСЕ similarity параметры ОСТАЮТСЯ неизменными!
                        // Ничего не удаляем и не перезаписываем!

                    } else {
                        // Similarity Filters - собираем значения из чекбоксов
                        const genreCheckboxes = document.querySelectorAll('.genre-checkbox:checked');
                        const genreIds = Array.from(genreCheckboxes).map(cb => cb.value);
                        ...
                        // Обновляем similarity параметры
                        if (genreIds.length > 0) {
                            currentUrl.searchParams.set('g', genreIds.join(','));
                        } else {
                            currentUrl.searchParams.delete('g');
                        }

                        if (keywordIds.length > 0) {
                            currentUrl.searchParams.set('k', keywordIds.join(','));
                        } else {
                            currentUrl.searchParams.delete('k');
                        }

                        if (themeIds.length > 0) {
                            currentUrl.searchParams.set('t', themeIds.join(','));
                        } else {
                            currentUrl.searchParams.delete('t');
                        }

                        if (perspectiveIds.length > 0) {
                            currentUrl.searchParams.set('pp', perspectiveIds.join(','));
                        } else {
                            currentUrl.searchParams.delete('pp');
                        }

                        if (gameModeIds.length > 0) {
                            currentUrl.searchParams.set('gm', gameModeIds.join(','));
                        } else {
                            currentUrl.searchParams.delete('gm');
                        }

                        if (engineIds.length > 0) {
                            currentUrl.searchParams.set('e', engineIds.join(','));
                        } else {
                            currentUrl.searchParams.delete('e');
                        }
                    }

                    // Устанавливаем page=1
                    ...
                    // Убеждаемся что find_similar=1
                    currentUrl.searchParams.set('find_similar', '1');

                    console.log('Final URL:', currentUrl.toString());
                    console.log('All params:', Array.from(currentUrl.searchParams.entries()));
